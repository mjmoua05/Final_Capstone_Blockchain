...
contract SolnSquareVerifier is CustomERC721Token {
  ...
  function addSolution(uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[2] memory input) internal {
        bytes32 key = keccak256(abi.encodePacked(a, b, c, input));
        require(uniqueSolutions[key].index == 0, "Solution already exsits");
        uniqueSolutions[key] = Solution({index: solCount, to: msg.sender});
        solCount += 1;
  ...
  function mintToken(address _to, uint256 _index, uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[2] memory input) public {
    bytes32 key = keccak256(abi.encodePacked(a, b, c, input));
    require(uniqueSolutions[key].to == address(0), "Solution already exists!");
    require(verifierContract.verifyTx(a, b, c, input), "Solution not correct!");
